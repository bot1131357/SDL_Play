...
public:
	...
	void update()
	{
		...
		if (x < 0) if (_v_x <0)  _v_x = -_v_x;
		if (x + dstRect.w > width) if (_v_x > 0) _v_x = -_v_x;
		...
		const float MIN_VEL = 50/ pollrate;
		// Deccelerate
...
public:
	...
	float get_v_x() { return _v_x; }
	float get_v_y() { return _v_y; }
 ...
 bitset<5> detectCollision(SDLEntity &ref, SDLEntity &obj)
{
	...
	if (ref._v_x>obj._v_x) 
		if (ref.dstRect.x + ref.dstRect.w > obj.dstRect.x)  // ref right bound is more than obj left bound
			if (ref.dstRect_.x + ref.dstRect_.w < obj.dstRect.x)  // ref right bound is previously less than obj left bound 
				if (ref.dstRect.y + ref.dstRect.h > obj.dstRect.y)  // ref lower bound is lower than obj upper bound
					if (ref.dstRect.y < obj.dstRect.y + obj.dstRect.h) // ref upper bound is higher than obj lower bound
						res.set(Collision::Right);

	if (ref._v_x<obj._v_x)
		if (ref.dstRect.x < obj.dstRect.x + obj.dstRect.w)  // ref left bound is less than obj right bound
			if (ref.dstRect_.x > obj.dstRect.x + obj.dstRect.w)  // ref left bound is previously more than obj right bound 
				if (ref.dstRect.y + ref.dstRect.h > obj.dstRect.y)  // ref lower bound is lower than obj upper bound
					if (ref.dstRect.y < obj.dstRect.y + obj.dstRect.h) // ref upper bound is higher than obj lower bound
	...
	if (ref._v_y>obj._v_y)
		if (ref.dstRect.y + ref.dstRect.h > obj.dstRect.y) // ref lower bound is lower than obj upper bound
			if (ref.dstRect_.y + ref.dstRect_.h < obj.dstRect.y) // ref lower bound is previously higher than obj upper bound 
				if (ref.dstRect.x + ref.dstRect.w > obj.dstRect.x) // ref right bound is more than obj left bound
					if (ref.dstRect.x < obj.dstRect.x + obj.dstRect.w) // ref left bound is less than obj right bound
	...
	if (ref._v_y<obj._v_y)
		if (ref.dstRect.y < obj.dstRect.y + obj.dstRect.h)  // ref upper bound is higher than obj lower bound
			if (ref.dstRect_.y > obj.dstRect.y + obj.dstRect.h)  // ref upper bound is previously lower than obj lower bound 
				if (ref.dstRect.x + ref.dstRect.w > obj.dstRect.x)  // ref right bound is more than obj left bound
					if (ref.dstRect.x < obj.dstRect.x + obj.dstRect.w) // ref left bound is higher than obj right bound
